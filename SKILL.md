...
5. **🚫 禁止 PM 指定具体查询命令或增加额外工作** — 派发 subagent 时，PM 仅传递必要的关键信息（traceId、corpId、环境、排查意图、已排查的部分结果等一切必要参数信息），不得指定具体的查询命令（如"执行 SearchLog"、"使用 DescribeGeneralSpanList"），不得自主增加 subagent 职责范围外的其他工作。subagent 内部自行决定使用何种工具和命令完成其职责。

> **违反以上任一条均视为流程错误。** 当 PM 检测到自身有"直接查日志"、"直接执行命令"或"在派发 prompt 中指定具体查询命令/增加额外工作"的冲动时，应立即停止并转为派发 subagent（仅传递关键信息）。
...
当决策启用日志抓取时，PM 优先派发 `tencent-cloud-troubleshooter` subagent（不可用时按降级策略创建或使用默认 subagent），prompt 中仅传递以下关键信息：

1. **环境**（根据 Step 1.5.1.5 环境选择规则确定）
2. **检索键**（traceId 优先，无 traceId 时用 corpId）
...
```
请协助查询以下环境的日志：
- 环境: [生产(ap-shanghai) / 测试(ap-chengdu)]
- traceId: [如有]
- corpId: [如无 traceId 则尽量获取]
- 异常信息摘要: [已知的异常摘要]
- 时间范围: [如知道则提供，否则留空]
- 排查意图: [如：确认异常根因/查看业务参数流转/了解调用链路等]
- 已排查的部分结果: [如有，填写已掌握的信息]
```
...
1. **派发后端专家**：传递异常提示信息、日志排查结果、排查意图（理解业务异常触发原因），由后端专家自行决定排查方式
2. **派发前端专家**（如需要）：传递异常提示信息、用户操作路径，由前端专家自行决定排查方式
...
派发给 subagent 时，prompt 仅传递以下关键信息：
...
- 关键线索：[堆栈/截图/错误码等]
- corpId：[如有]
- 日志排查结果：[如执行了 Step 1.5，填写关键结论；未执行则填"N/A"]
- 已排查的部分结果：[如有]
```
...
派发给 `frontend-bug-fixer` subagent，prompt 中仅传递以下关键信息：
- **工作目录**：`E:\workspace\handday-web`
- **权限声明**：仅允许读取和搜索，禁止新增文件和提交
- **Bug 信息摘要**（按 Step 2 分派指令模板格式）
- **如版本不符**：暂停并回报 PM，由 PM 向用户确认分支

### 3.2 后端专家工作内容

派发给 `senior-java-expert` subagent，prompt 中仅传递以下关键信息：
- **Bug 信息摘要**（按 Step 2 分派指令模板格式）
- **日志排查结果**（如执行了 Step 1.5，将日志专家返回的关键结论一并传递）
- **前端定位结果**（如前端已排查，将前端专家返回的接口信息一并传递）
